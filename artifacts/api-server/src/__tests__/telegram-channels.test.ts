  ...
  () => {
    ...
    beforeAll(async () => {
      ...
      const [admin] = await db
        .insert(usersTable)
        .values({
          name: `tg_admin_${Date.now()}`,
          email: `tg_admin_${Date.now()}@t.pe`,
          role: "super_admin",
          sector: "T",
          district: "T",
          districtId,
    ...
    afterAll(async () => {
      ...
      await db.execute(
        sql`DELETE FROM "districts" WHERE "name" = 'TG District'`,
      );
    });
    ...
    it("webhook ignora códigos inválidos (no cambia nada)", async () => {
      const res = await request(app)
        .post("/api/telegram/webhook")
        .send({
          channel_post: { chat: { id: -1 }, text: "/vincular NOEXISTE99" },
        });
      expect(res.status).toBe(200);
      const [d] = await db
        .select({ chatId: districtsTable.telegramChatId })
    ...
    it("webhook exige el secret cuando está configurado", async () => {
      ...
      const res = await request(app)
        .post("/api/telegram/webhook")
        .send({
          channel_post: { chat: { id: -5 }, text: `/vincular ${code}` },
        });
