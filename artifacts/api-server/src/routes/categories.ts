...
router.post("/departments", requireAuth, requireAdmin, async (req, res) => {
  const districtId = getDistrictId(req);
  if (!districtId)
    return res.status(400).json({ error: "Se requiere distrito." });
...
router.patch(
  ...
  async (req, res) => {
    ...
    const parsed = z
      .object({ departmentId: z.number().int().nullable() })
      .safeParse(req.body);
    if (!parsed.success)
      return res
        .status(400)
        .json({ error: "departmentId requerido (número o null)." });
    ...
    try {
      ...
      if (!checkTenant(req, report.districtId))
        return res
          .status(403)
          .json({ error: "No puedes asignar reportes de otro distrito." });

      const previousDept = report.assignedTo;
      await db
        .update(reportsTable)
        .set({ assignedTo: parsed.data.departmentId })
        .where(eq(reportsTable.id, reportId));
      ...
      await db.insert(auditLogTable).values({
        ...
        newValue: parsed.data.departmentId
          ? String(parsed.data.departmentId)
          : null,
...
router.post("/subscriptions", async (req, res) => {
  ...
  if (!parsed.success)
    return res
      .status(400)
      .json({ error: parsed.error.issues.map((i) => i.message).join("; ") });
  if (!parsed.data.fcmToken && !parsed.data.email)
    return res.status(400).json({ error: "Se requiere fcmToken o email." });
  ...
  try {
    ...
    const existing = parsed.data.fcmToken
      ? await db
          ...
          .where(eq(subscriptionsTable.fcmToken, parsed.data.fcmToken))
          .limit(1)
      : [];
