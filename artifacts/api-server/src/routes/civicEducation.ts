...
router.get("/civic-education/topics", async (_req, res) => {
  try {
    ...
    return res.json({
      topics: topics.map((t) => ({ ...t, questions: JSON.parse(t.questions) })),
    });
  } catch (err) {
...
router.get("/civic-education/topics/:slug", async (req, res) => {
  try {
    const [topic] = await db
      ...
      .where(
        and(
          eq(civicEducationTopicsTable.slug, req.params.slug as string),
          eq(civicEducationTopicsTable.isActive, true),
        ),
      )
      .limit(1);
    if (!topic)
...
router.post("/civic-education/progress", optionalAuth, async (req, res) => {
  ...
  try {
    const [existing] = await db
      ...
      .where(
        and(
          eq(userEducationProgressTable.userId, userId),
          eq(userEducationProgressTable.topicId, parsed.data.topicId),
        ),
      )
      .limit(1);
    ...
    if (existing) {
      const [updated] = await db
        .update(userEducationProgressTable)
        .set({
          ...
          completedAt: parsed.data.completed
            ? new Date()
    ...
    const [progress] = await db
      .insert(userEducationProgressTable)
      .values({
        userId,
