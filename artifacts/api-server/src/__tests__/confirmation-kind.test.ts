...
describe.skipIf(!process.env.DATABASE_URL)(
  "Confirmaciones validity/resolution",
  () => {
    let app: any;
    let request: any;
    let db: any;
    ...
    afterAll(async () => {
      if (!db) return;
      await db.execute(
        sql`DELETE FROM "resolution_confirmations" WHERE "report_id" = ${reportId}`,
      );
    ...
    it("un mismo vecino puede confirmar VALIDEZ y RESOLUCIÓN (no chocan)", async () => {
      ...
      const r = await request(app)
        .post(`/api/reports/${reportId}/confirm-resolution`)
        .send({});
