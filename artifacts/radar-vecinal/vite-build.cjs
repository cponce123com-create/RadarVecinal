...
// Pre-load ALL deps that Vite needs so the CJS/ESM module cache has them
const preload = [
  "esbuild", "rollup", "react", "react-dom", "react-dom/client",
  "react-dom/server", "@vitejs/plugin-react", "@tailwindcss/vite",
  "vite-plugin-pwa", "workbox-window", "framer-motion",
  "lucide-react", "recharts", "date-fns", "wouter",
  "react-leaflet", "leaflet", "clsx", "tailwind-merge",
  "class-variance-authority", "react-hot-toast",
  // Radix UI and shadcn deps
];
const startDir = __dirname;
for (const pkg of preload) {
  try {
    const p = resolveFromPnpm(pkg, startDir);
    require(p);
  } catch {}
}

// Now load vite and run build
...
const config = defineConfig({
  ...
  plugins: [react(), tailwindcss(), VitePWA({
    ...
    manifest: {
      name: "Radar Vecinal", short_name: "RadarV",
      description: "Plataforma vecinal de seguridad y reportes ciudadanos",
      theme_color: "#060810", background_color: "#060810",
      display: "standalone", orientation: "portrait",
      start_url: "/", scope: "/", lang: "es-PE",
