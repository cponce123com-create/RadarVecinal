...
describe.skipIf(!process.env.DATABASE_URL)(
  ...
  () => {
    ...
    it("M-05: Debe crear distritos de prueba", async () => {
      // Crear distrito A
      const [dA] = await db.insert(districtsTable).values({
      ...
      // Crear distrito B
      const [dB] = await db.insert(districtsTable).values({
    ...
    it("M-04/M-06: Debe crear usuarios de prueba con tokens", async () => {
      ...
      adminAToken = jwt.sign(
        { sub: String(adminA.id), email: adminA.email, role: "admin", districtId: districtAId },
        process.env.JWT_SECRET!,
        { expiresIn: "1h" }
      );

      // Admin B (distrito B)
      const [adminB] = await db.insert(usersTable).values({
        name: "Admin B",
      ...
      adminBToken = jwt.sign(
        { sub: String(adminB.id), email: adminB.email, role: "admin", districtId: districtBId },
        process.env.JWT_SECRET!,
        { expiresIn: "1h" }
      );

      // Super Admin (sin distrito específico)
      const [superA] = await db.insert(usersTable).values({
        name: "Super Admin",
      ...
      superAdminToken = jwt.sign(
        { sub: String(superA.id), email: superA.email, role: "super_admin", districtId: districtAId },
        process.env.JWT_SECRET!,
        { expiresIn: "1h" }
      );
      ...
      userToken = jwt.sign(
        { sub: String(user.id), email: user.email, role: "user", districtId: districtAId },
        process.env.JWT_SECRET!,
        { expiresIn: "1h" }
      );
    ...
    it("M-01: Reportes deben estar aislados por distrito", async () => {
      ...
      const [rA] = await db.insert(reportsTable).values({
        title: "Reporte A",
      ...
      const [rB] = await db.insert(reportsTable).values({
        title: "Reporte B",
      ...
      const reportsA = await db.select()
        .from(reportsTable)
        .where(eq(reportsTable.districtId, districtAId));

      const reportsB = await db.select()
        .from(reportsTable)
      ...
      expect(reportsA.every((r: any) => r.districtId === districtAId)).toBe(true);
      expect(reportsB.every((r: any) => r.districtId === districtBId)).toBe(true);
    ...
    it("M-04: Admin de distrito A NO debe poder modificar reportes de distrito B", async () => {
      ...
      const canModify = payloadA.role === "super_admin" || Number(payloadA.districtId) === Number(reportB.districtId);
    ...
    it("M-04: Super admin SÍ debe poder acceder a cualquier distrito", async () => {
      ...
      const payload = jwt.verify(superAdminToken, process.env.JWT_SECRET!) as any;
      const canAccess = payload.role === "super_admin" || Number(payload.districtId) === Number(reportB.districtId);
    ...
    it("M-01: GET /api/reports sin auth debe filtrar por districtId explícito", async () => {
      ...
      expect(reports.every((r: any) => r.districtId === districtIdFromQuery)).toBe(true);
    });
      ...
      if (db) {
        ...
        try {
          await db.delete(reportsTable)
            .where(sql`${reportsTable.districtId} IN (${districtAId}, ${districtBId})`);
          await db.delete(panicAlertsTable)
            .where(sql`${panicAlertsTable.districtId} IN (${districtAId}, ${districtBId})`);
          await db.delete(missingPersonsTable)
            .where(sql`${missingPersonsTable.districtId} IN (${districtAId}, ${districtBId})`);
