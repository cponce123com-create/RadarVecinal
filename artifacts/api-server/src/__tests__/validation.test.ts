...
const createReportSchema = z.object({
  title: z.string().min(5, 'El título debe tener al menos 5 caracteres').max(160),
  description: z.string().min(10, 'La descripción debe tener al menos 10 caracteres').max(2000),
  category: z.enum(['robbery','fight','suspicious','water_cut','garbage','noise','fire','medical_emergency','missing_person','prostitution','drug_point','bar_trouble','informal_commerce','other']),
  ...
  contactPhone: z.string().regex(/^\d{6,15}$/, 'Teléfono inválido').optional().nullable(),
