...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename=\"reportes-${districtSlug}-${new Date().toISOString().slice(0, 10)}.pdf\"`,
      );
      doc.pipe(res);
      ...
      doc
        .fontSize(18)
        .font("Helvetica-Bold")
        .text(`Reportes Ciudadanos - ${districtName}`, { align: "center" });
      doc
        .fontSize(9)
        .font("Helvetica")
        .fillColor("#64748b")
        .text(
          `Generado el ${new Date().toLocaleDateString("es-PE")} · ${total} reportes`,
          { align: "center" },
        );
      doc.moveDown(0.8);
      ...
      doc.text(
        `Total: ${total}  |  Resueltos: ${resolved}  |  Pendientes: ${total - resolved}`,
      );
      doc.moveDown(0.3);
      ...
      reports.forEach((r, i) => {
        ...
        doc.text(
          `${CAT_LABELS[r.category] ?? r.category} · ${r.sector} · ${STATUS_LABELS[r.status] ?? r.status} · ${new Date(r.createdAt).toLocaleDateString("es-PE")}`,
        );
        if (r.description) {
          doc
            .fillColor("#475569")
            .text(r.description.slice(0, 200), { indent: 10 });
        }
        doc.moveDown(0.4);
      });
      ...
      doc
        .fontSize(8)
        .fillColor("#94a3b8")
        .text("— Radar Vecinal · radarvecinal.pe —", { align: "center" });
