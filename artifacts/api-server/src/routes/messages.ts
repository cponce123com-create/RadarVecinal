    ...
    try {
      ...
      if (!checkTenant(req, report.districtId)) {
        return res.status(403).json({
          error: "No puedes enviar mensajes a reportes de otro distrito.",
        });
      }
      ...
      if (channel === "whatsapp" && !hasPhone) {
        return res.status(400).json({
          error: "Este reporte no tiene número de contacto registrado.",
        });
      }
      if (channel === "email" && !hasEmail) {
        return res.status(400).json({
          error: "Este reporte no tiene correo electrónico registrado.",
        });
      }
        ...
        deliverPromises.push(
          sendCustomMessageEmail({
            ...
            logger.error(
              ...
              "[Messages] Error al enviar email",
            );
          }),
        );
      }
      ...
      if (channel === "app") {
        ...
        await db
          ...
          .catch((err) => {
            logger.error(
              { err, reportId: report.id },
              "[Messages] Error al crear notificación app",
            );
          });
        ...
        if (authorUserId) {
          deliverPromises.push(
            sendMunicipalPushToUser({
              ...
              logger.error(
                { err, reportId: report.id },
                "[Messages] Error al enviar push al vecino",
              );
            }),
          );
        }
  ...
  async (req, res) => {
    try {
      ...
      if (!checkTenant(req, report.districtId)) {
        return res
          .status(403)
          .json({ error: "No puedes ver mensajes de otros distritos." });
      }
